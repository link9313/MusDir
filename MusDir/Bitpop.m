...
@implementation Bitpop

SPTListPage *searchBitpopResults;
NSArray *bitpopResults;
NSArray *bitpopURIs;

- (void)viewDidLoad {
    [super viewDidLoad];
    // Search for bitpop tracks
    [SPTSearch performSearchWithQuery:@"bitpop" queryType:SPTQueryTypeTrack accessToken:nil
                             callback:^(NSError *searchError, SPTListPage *searchBitpopResults)
    {
        ...
        for (int x = 0; x < [[searchBitpopResults items] count]; x++) {
            // Grab the track in each position of the items NSArray
            SPTPartialTrack *track = [[searchBitpopResults items] objectAtIndex:x];
        ...
        bitpopResults = [NSArray arrayWithArray:arrayOfTracks];
        bitpopURIs = [NSArray arrayWithArray:arrayofURIs];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = [bitpopResults objectAtIndex:indexPath.row];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *message = [bitpopResults objectAtIndex:indexPath.row];
    NSString *URI = [NSString stringWithFormat:@"%@", [bitpopURIs objectAtIndex:indexPath.row]];
