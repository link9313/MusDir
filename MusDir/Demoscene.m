...
SPTListPage *searchDemosceneResults;
NSArray *demosceneResults;
NSArray *demosceneURIs;
...
- (void)viewDidLoad {
    ...
    // Search for demoscene tracks
    [SPTSearch performSearchWithQuery:@"demoscene" queryType:SPTQueryTypeTrack accessToken:nil
                             callback:^(NSError *searchError, SPTListPage *searchDemosceneResults)
    {
        ...
        for (int x = 0; x < searchDemosceneResults.items.count; x++) {
            // Grab the track in each position of the items NSArray
            SPTPartialTrack *track = [searchDemosceneResults.items objectAtIndex:x];
        ...
        demosceneResults = [NSArray arrayWithArray:arrayOfTracks];
        demosceneURIs = [NSArray arrayWithArray:arrayofURIs];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return demosceneResults.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Cell" forIndexPath:indexPath];
    
    cell.textLabel.text = [demosceneResults objectAtIndex:indexPath.row];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *message = [demosceneResults objectAtIndex:indexPath.row];
    NSString *URI = [NSString stringWithFormat:@"%@", [demosceneURIs objectAtIndex:indexPath.row]];
